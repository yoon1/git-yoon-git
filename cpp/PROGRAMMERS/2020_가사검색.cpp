	...
	int find(const char* key) {
		...
		if (*key == '?') {
			...
			for (int k = 0; k < 26; k++) {
				cout << k << endl;
				if (next[k] != NULL) {
					cout << "tmp : " << tmp << "count : " << next[k]->count << endl;
					tmp += next[k]->count;
				}
			}
			cout << "key : " << *key << "temp : " << tmp << endl;
			return tmp;
		}
...
int main() {
	//vector<int> rslt = solution({ "frodoe", "front", "frost", "frozen", "frame", "kakao" },
	//   {"fro??", "????o", "fr???", "fro???", "pro?", "?????"});
	vector<int> rslt = solution({ "frodo","frame", "front", "kakao", "kak" }
		,	{ "????", "?????" });
