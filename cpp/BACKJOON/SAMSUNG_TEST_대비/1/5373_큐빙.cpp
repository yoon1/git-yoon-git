...
int plane_dat[8] = { 0,1,2,5,8,7,6,3 }; // 면 좌표?
	...
	for (int testCase = 0; testCase < T; testCase++) {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int i = 0; i < 12; i++) {
				...
				t[i] = cube[tdat[i].pid].d[tdat[i].did - 1]; // 값의 경우 1~9을 기준으로 했기 때문에 -1을 함
			}
			
			for (int i = 0; i < 8; i++) {
				tplane[i] = cube[inm].d[plane_dat[i]]; // 0번째의 d[[
			}
			...
			// 다시 놓기
			for (int i = 0; i < 12; i++) {
				cube[tdat[i].pid].d[tdat[i].did - 1] = t[i];
			}

			
			for (int i = 0; i < 8; i++) {
				cube[inm].d[plane_dat[i]] = tplane[i];
			}
