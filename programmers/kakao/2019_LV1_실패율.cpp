...
vector<int> solution(int N, vector<int> stages) {
	...
	for (int i = 1; i < size; i++)// 초기 index 설정
		cnt_stage[i].num = i;
	for (int i = 0; i < stages.size(); i++)  // counting sort
		cnt_stage[stages[i]].fail_cnt++;
	for (int i = 1; i < cnt_stage.size()-1; i++) { // user_cnt 구하기
		cnt_stage[i].user_cnt = tot_user;
		tot_user -= cnt_stage[i].fail_cnt;
	}
	sort(cnt_stage.begin()+1, cnt_stage.end()-1, cmp); // sorting(내림차순)
	for (int i = 1; i < size - 1; i++)  // 정렬된 값 answer 추가
